...
int main(int argc, char** argv)
{
    ...
    if (!config.cliRenderingEnabled) {
        ...
        //Debugging for SAH
        //numSplits should be the same as in bounding_volume_hierarchy.cpp
        #define numSplits 5
        int splitSAH = 0;
        int levelSAH = 0;
        bool debugForSAH = { false };

        //DOF variables
        config.features.dof.focal_length = MIN_FOCAL_LENGTH;
        config.features.dof.aperture = MIN_APERTURE;
        ...
        while (!window.shouldClose()) {
            ...
            if (ImGui::CollapsingHeader("Extra Features")) {
                ...
                if (config.features.extra.enableBloomEffect) {
                    ...
                    ImGui::SliderInt("FilterSize", &filtersize, 1, 10);
                    ImGui::Checkbox("Threshold only", &config.features.extra.enableThreshold);
                    ImGui::Checkbox("BoxFilter only", &config.features.extra.enableBoxFilter);
                    ImGui::Checkbox("Scale only", &config.features.extra.enableScale);
                }
            ...
            if (viewMode == ViewMode::Rasterization) {
                ...
                if (config.features.enableAccelStructure)
                    ImGui::Checkbox("Draw Extra Boxes", &config.features.enableExtraBoxes);
                if (config.features.extra.enableBvhSahBinning) {
                    ImGui::Separator();
                    ImGui::Text("SAH Debug");
                    ImGui::Checkbox("Draw splits", &debugForSAH);
                    if (debugForSAH) {
                        ImGui::SliderInt("BVH level", &levelSAH, 0, bvh.numLevels() - 1);
                        ImGui::SliderInt("SAH split", &splitSAH, 0, numSplits - 1);
                    }
                }
            ...
            if (config.features.extra.enableDepthOfField) {
                dof = true;
                ImGui::Separator();
                ImGui::Text("Deapth Of Field");
                ImGui::SliderFloat("Focal Length", &config.features.dof.focal_length, MIN_FOCAL_LENGTH, MAX_FOCAL_LENGTH);
                ImGui::SliderFloat("Aperture", &config.features.dof.aperture, MIN_APERTURE, MAX_APERTURE);
            }

            /*ImGui::Spacing();
            ImGui::Separator();
            ImGui::Text("Debugging Bloom Effect");
            if (viewMode == ViewMode::RayTracing) {
                ImGui::Checkbox("View Threshold", );
            }*/
            ...
            ImGui::Text("Lights");
            {
                ImGui::SliderInt("Number of light samples", &config.features.numberOfSamples, 1, 500);
            ...
            case ViewMode::Rasterization: {
                ...
                #ifdef DOFdebug
                if (dof) {
                    drawSphereWireFrame(camera.position(), config.features.dof.focal_length, glm::vec3(0.6f));
                }
                #endif


                if (debugBVHLevel || debugBVHLeaf || debugForSAH) {
                    glPushAttrib(GL_ALL_ATTRIB_BITS);
                    setOpenGLMatrices(camera);
                    glDisable(GL_LIGHTING);
                    ...
                    if (debugForSAH) {
                        bvh.debugSAH(levelSAH, splitSAH);
                    }
                    enableDebugDraw = false;
                    glPopAttrib();
                }
